part of 'BlockBuilder2.0.dart';

/// Helper to parse Firestore day documents into typed data
class DayPayload {
  final List<Map<String, dynamic>> exercises;
  final List<int> circuits;

  DayPayload({required this.exercises, required this.circuits});

  factory DayPayload.fromRaw(Map<String, dynamic> raw) {
    final exRaw = raw['exercises'] as List<dynamic>? ?? [];
    final exercises = exRaw
        .whereType<Map>()
        .map((m) => Map<String, dynamic>.from(m))
        .toList();

    final cirRaw = raw['circuitStartIndices'] as List<dynamic>? ?? [];
    final circuits = cirRaw.whereType<int>().toList();

    return DayPayload(exercises: exercises, circuits: circuits);
  }
}

/// Mixin providing block-data loading into the private state class
mixin BlockBuilderDataLoader on _BlockBuilder2State {
  List<List<List<ExerciseRow>>> get exerciseRows;
  set exerciseRows(List<List<List<ExerciseRow>>> v);

  List<List<List<int>>> get circuitStartIndices;
  set circuitStartIndices(List<List<List<int>>>> v);

  String? get _selectedBlockId; // private is fine in a part
  /// Load all weeks & days, parse with DayPayload, then set state once
  Future<void> loadBlockDataFromFirestore() async {
    final user = FirebaseAuth.instance.currentUser;
    if (user == null) return;

    final weekSnaps = await FirebaseFirestore.instance
        .collection('users')
        .doc(user.uid)
        .collection('block_data')
        .doc('current_block')
        .collection('weeks')
        .get();

    await Future.wait(weekSnaps.docs.map((weekDoc) async {
      final weekIndex = int.parse(weekDoc.id.replaceFirst('week_', ''));
      final daySnaps = await weekDoc.reference.collection('days').get();

      for (final dayDoc in daySnaps.docs) {
        final dayIndex = int.parse(dayDoc.id.replaceFirst('day_', ''));
        final rawDay = dayDoc.data();
        if (rawDay == null) continue;

        final payload = DayPayload.fromRaw(Map<String, dynamic>.from(rawDay));
        final rows = payload.exercises.map((ex) {
          final row = ExerciseRow(
            exercise: ex['name'] as String? ?? '',
            circuitIndex: ex['circuitIndex'] as int? ?? 0,
          );
          final w = (ex['weight'] as num?)?.toDouble() ?? 0.0;
          final r = (ex['reps'] as num?)?.toInt() ?? 0;
          final i = (ex['rir'] as num?)?.toDouble() ?? 0.0;
          if (w > 0) row.weightController.text = w.toString();
          if (r > 0) row.repsController.text = r.toString();
          if (i > 0) row.rirController.text = i.toString();
          return row;
        }).toList();

        exerciseRows[weekIndex][dayIndex] = rows;
        circuitStartIndices[weekIndex][dayIndex] = payload.circuits;
      }
    }));

    setState(() {});
  }

  /// Load a single week (and optional overrides), then set state once
  Future<void> loadBlockDataForWeek(int weekIndex) async {
    final user = FirebaseAuth.instance.currentUser;
    if (user == null) return;

    final weekRef = FirebaseFirestore.instance
        .collection('planned_blocks')
        .doc(user.uid)
        .collection('blocks')
        .doc(_selectedBlockId!)
        .collection('weeks')
        .doc('week_$weekIndex');

    final weekSnap = await weekRef.get();
    if (!weekSnap.exists) return;

    final daySnaps = await weekRef.collection('days').get();

    await Future.wait(daySnaps.docs.map((dayDoc) async {
      final dayIndex = int.tryParse(dayDoc.id.replaceFirst('day_', '')) ?? 0;
      final raw = dayDoc.data();
      if (raw == null) return;

      final payload = DayPayload.fromRaw(Map<String, dynamic>.from(raw));
      final rows = payload.exercises.map((ex) {
        final row = ExerciseRow(
          id: const Uuid().v4(),
          exercise: ex['name'] as String? ?? '',
          circuitIndex: ex['circuitIndex'] as int? ?? 0,
        );
        row.exerciseController.text = row.exercise!;
        final wVal = (ex['weight'] as num?)?.toDouble();
        final rVal = (ex['reps'] as num?)?.toInt();
        final iVal = (ex['rir'] as num?)?.toDouble();
        if (wVal != null && wVal > 0)
          row.weightController.text = wVal.toString();
        if (rVal != null && rVal > 0) row.repsController.text = rVal.toString();
        if (iVal != null && iVal > 0) row.rirController.text = iVal.toString();
        return row;
      }).toList();

      exerciseRows[weekIndex][dayIndex] = rows;
      circuitStartIndices[weekIndex][dayIndex] = payload.circuits;

      final workoutDoc =
          await weekRef.collection('days').doc('day_$dayIndex').get();
      if (!workoutDoc.exists) return;
      final rawOv = workoutDoc.data();
      if (rawOv == null) return;

      final overridePayload =
          DayPayload.fromRaw(Map<String, dynamic>.from(rawOv));
      for (final ex in overridePayload.exercises) {
        final name = ex['name'] as String? ?? '';
        final circuit = ex['circuitIndex'] as int? ?? 0;
        final sets = List<Map<String, dynamic>>.from(ex['sets'] ?? []);
        final matching = rows.firstWhere(
          (r) => r.exercise == name && r.circuitIndex == circuit,
          orElse: () => null,
        );
        if (matching != null && sets.isNotEmpty) {
          matching.weightController.text = sets[0]['weight']?.toString() ?? '';
          matching.repsController.text = sets[0]['reps']?.toString() ?? '';
          matching.rirController.text = sets[0]['rir']?.toString() ?? '';
        }
      }
    }));

    setState(() {});
  }
}
